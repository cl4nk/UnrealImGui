...
public:
	...
	UFUNCTION(BlueprintCallable, meta = (Category = "ImGui|Windows")) static bool          Begin(FString name, UPARAM(meta = (Bitmask, BitmaskEnum = EImGuiWindowFlags_)) int32 flags = 0);
	UFUNCTION(BlueprintCallable, meta = (Category = "ImGui|Windows")) static bool          BeginWithOpen(FString name, UPARAM(ref) bool& p_open, UPARAM(meta = (Bitmask, BitmaskEnum = EImGuiWindowFlags_)) int32 flags = 0);
	...
	UFUNCTION(BlueprintCallable, meta = (Category = "ImGui|Widgets|Popups")) static bool          BeginPopupModal(FString name, UPARAM(meta = (Bitmask, BitmaskEnum = EImGuiWindowFlags_)) int32 flags = 0);                     // modal dialog (regular window with title bar, block interactions behind the modal window, can't close the modal window by clicking outside)
	UFUNCTION(BlueprintCallable, meta = (Category = "ImGui|Widgets|Popups")) static bool          BeginPopupModalWithOpen(FString name, UPARAM(ref) bool& p_open, UPARAM(meta = (Bitmask, BitmaskEnum = EImGuiWindowFlags_)) int32 flags = 0);                     // modal dialog (regular window with title bar, block interactions behind the modal window, can't close the modal window by clicking outside)
